...
const MortgageCalculator: React.FC = () => {
  ...
  const repaymentOptions = [
    { value: 'equal-principal-interest', label: t('finance.mortgage.opt_equal_principal_interest') },
    { value: 'equal-principal', label: t('finance.mortgage.opt_equal_principal') }
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-md space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-600 mb-2">{t('finance.mortgage.label_principal')}</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-600 mb-2">{t('finance.mortgage.label_rate')}</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-600 mb-2">{t('finance.mortgage.label_period')}</label>
        ...
        <SelectModal
          label={t('finance.mortgage.label_method')}
      ...
      {result && (
        <div className="p-6 bg-gray-50 rounded-xl text-left space-y-3">
          <div className="flex justify-between items-center text-blue-600">
            <span className="font-semibold">{t('finance.mortgage.result_monthly_payment')}</span>
            <span className="font-bold text-xl">{result.monthlyPayment} {t('currency.KRW')}</span>
          </div>
          <hr className="my-2" />
          <div className="flex justify-between items-center">
            <span className="text-gray-600">{t('finance.mortgage.result_total_principal')}</span>
            <span className="font-semibold text-lg">{parseFloat(loanAmount.replace(/,/g, '')).toLocaleString()} {t('currency.KRW')}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-gray-600">{t('finance.jeonse.result_total_interest')}</span>
            <span className="font-semibold text-lg">{result.totalInterest.toLocaleString()} {t('currency.KRW')}</span>
          </div>
          <div className="flex justify-between items-center mt-2">
            <span className="font-semibold">{t('finance.mortgage.result_total_repayment')}</span>
