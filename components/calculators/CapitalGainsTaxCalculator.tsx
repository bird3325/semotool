    ...
    <div className="space-y-6">
      <div className="p-6 rounded-2xl text-white shadow-lg bg-gradient-to-br from-violet-400 to-violet-600">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-2xl font-bold">{t('tool.capital-gains-tax')} {t('suffix.calculator')}</h2>
        </div>
        <p className="mt-1 opacity-90">{t('finance.capital_gains.desc')}</p>
      </div>

      <div className="bg-white p-6 rounded-xl shadow-md space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-600 mb-2">{t('finance.capital_gains.label_purchase_price')}</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-600 mb-2">{t('finance.capital_gains.label_sale_price')}</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-600 mb-2">{t('finance.capital_gains.label_holding_period')}</label>
      ...
      {result && (
        <div className="p-6 bg-gray-50 rounded-xl text-center">
          <p className="text-sm text-gray-500">{t('finance.capital_gains.result_tax')}</p>
          <p className="text-4xl font-bold text-blue-600 my-2">
            {Math.round(result.tax).toLocaleString()} {t('currency.KRW')}
          </p>
          <p className="text-md text-gray-700">
            {t('finance.capital_gains.result_gain')}: {result.capitalGain.toLocaleString()}{t('currency.KRW')}
          </p>
          <p className="text-xs text-gray-500 text-center pt-2">{t('finance.capital_gains.msg_warning')}</p>
        </div>
      )}
    </div>
  );
