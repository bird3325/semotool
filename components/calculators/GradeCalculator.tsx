...
const GradeCalculator: React.FC = () => {
  ...
  const creditOptions = [
    { value: '1', label: `1${t('school.gpa.label_credits')}` },
    { value: '2', label: `2${t('school.gpa.label_credits')}` },
    { value: '3', label: `3${t('school.gpa.label_credits')}` },
    { value: '4', label: `4${t('school.gpa.label_credits')}` },
    { value: '5', label: `5${t('school.gpa.label_credits')}` },
  ];
    ...
    <div className="space-y-6">
      <div className="p-6 rounded-2xl text-white shadow-lg bg-gradient-to-br from-rose-400 to-rose-600">
        <div className="flex items-center space-x-3">
          <TrendingUp size={28} />
          <h2 className="text-2xl font-bold">{t('tool.gpa')} {t('suffix.calculator')}</h2>
        </div>
        <p className="mt-1 opacity-90">{t('school.gpa.desc')}</p>
      </div>

      <div className="bg-white p-6 rounded-xl shadow-md space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-600 mb-2">{t('school.gpa.label_course_name')}</label>
          <input
            type="text"
            placeholder={t('school.gpa.placeholder_course_name')}
            value={courseName}
        ...
        <div className="grid grid-cols-2 gap-4">
          <SelectModal
            label={t('school.gpa.label_credits')}
          ...
          />
          <SelectModal
            label={t('school.gpa.label_grade')}
        ...
        <button onClick={handleAddCourse} className="w-full flex items-center justify-center p-4 bg-gray-100 hover:bg-gray-200 text-gray-700 font-black rounded-lg transition-all active:scale-95 shadow-sm">
          <Plus size={20} className="mr-2" /> {t('school.gpa.btn_add_course')}
        </button>
      </div>
      ...
      {courses.length > 0 && (
        <div className="bg-white p-4 rounded-xl shadow-md space-y-2">
          <div className="flex justify-between items-center mb-2 px-2">
            <h3 className="text-sm font-black text-gray-400 uppercase tracking-widest">{t('school.gpa.header_course_list')}</h3>
            <span className="text-[10px] font-bold text-rose-500 bg-rose-50 px-2 py-0.5 rounded-full">{t('school.gpa.count_course', { count: courses.length })}</span>
          </div>
          <div className="space-y-2">
            {courses.map(course => (
              <div key={course.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100 hover:border-rose-100 transition-colors">
                <span className="flex-1 font-bold text-gray-800">{course.name || t('school.gpa.label_course_name')}</span>
                <div className="flex items-center space-x-4">
                  <span className="text-sm font-bold text-gray-500">{course.credits}{t('school.gpa.label_credits')}</span>
          ...
          <button onClick={handleCalculate} className="w-full mt-6 p-4 bg-rose-500 hover:bg-rose-600 text-white font-black rounded-xl text-lg transition-transform active:scale-95 shadow-lg shadow-rose-100">
            {t('school.gpa.btn_calculate_gpa')}
          </button>
        </div>
      )}
      ...
      {result && (
        <div className="p-8 bg-white border border-rose-100 rounded-3xl shadow-xl text-center animate-in zoom-in-95 duration-500">
          <p className="text-sm font-black text-gray-400 uppercase tracking-widest mb-2">{t('school.gpa.result_header')}</p>
          <div className="inline-block px-6 py-2 bg-rose-50 rounded-full mb-4">
            <p className="text-rose-600 font-bold">{t('school.gpa.result_total_credits', { credits: result.totalCredits })}</p>
          </div>
